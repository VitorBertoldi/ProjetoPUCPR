	...
	<div class="w3-main w3-container" style="margin-left:270px;margin-top:117px;">
		<div class="w3-panel w3-padding-large w3-card-4 w3-light-grey">
			...
			<div class="w3-code cssHigh notranslate">
				...
				$sql = "SELECT Matricula, Nome, id_necessidade, Foto FROM aluno WHERE Matricula = $id";
				...
				if ($result = mysqli_query($conn, $sql)) {
					if(mysqli_num_rows($result) == 1){
						...
						$necessidade = $row['id_necessidade'];
						$matricula     = $row['Matricula'];
						$nome          = $row['Nome'];
						$foto          = $row['Foto'];
									
						// Faz Select na Base de Dados
						$sqlG = "SELECT id_necessidade, Nome_necessidade FROM necessidade";
						...
						if ($result = mysqli_query($conn, $sqlG)) {
							while ($row = mysqli_fetch_assoc($result)) {
								$selected = "";
								if ($row['id_necessidade'] == $necessidade)
									$selected = "selected";
								array_push($optionsEspec, "\t\t\t<option " . $selected . " value='". $row["id_necessidade"]."'>".$row["Nome_necessidade"]."</option>\n");
							}
						}
						...
						<tr>
							<td style="width:50%;">
							<p>
							<input type="hidden" id="Id" name="Id" value="<?php echo $matricula; ?>">
							<p>
							...
							<p><label class="w3-text-IE"><b>Necessidade</b>*</label>
							<select name="Necessidade" id="Necessidade" class="w3-input w3-border w3-light-grey " required>
